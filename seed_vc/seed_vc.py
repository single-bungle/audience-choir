...
if torch.cuda.is_available():
    ...
    print("device setting is cuda")
elif torch.backends.mps.is_available():
    device = torch.device("mps")
    print("device setting is mps")
else:
    device = torch.device("cpu")
    print("device setting is cpu")

fp16 = False

#def load_models(args): 에 mps가 64bit 연산 지원하지 않는 문제점 해결
def load_models(args):
    global fp16
    fp16 = args.fp16

    def cast_for_mps(model_or_tensor):
        return model_or_tensor.float() if device.type == "mps" else model_or_tensor
    ...
    for key in model:
        ...
        model[key] = cast_for_mps(model[key])
    ...
    campplus_model = cast_for_mps(campplus_model)
    ...
    if vocoder_type == 'bigvgan':
        ...
        bigvgan_model = cast_for_mps(bigvgan_model)
        ...
        hift_gen = cast_for_mps(hift_gen)
        vocoder_fn = hift_gen
        ...
        _ = [cast_for_mps(vocos[key].eval().to(device)) for key in vocos]
    ...
    if speech_tokenizer_type == 'whisper':
        ...
        whisper_model = cast_for_mps(whisper_model)
        ...
        output_layer = config['model_params']['speech_tokenizer'].get('output_layer', None)
        ...
        model.to(device)
        model.eval()
        model = cast_for_mps(model)

        def semantic_fn(waves_16k):
            ori_waves_16k_input_list = [waves_16k[bib].cpu().numpy() for bib in range(len(waves_16k))]
            ori_inputs = feature_extractor(ori_waves_16k_input_list, return_tensors="pt", return_attention_mask=True, padding=True, sampling_rate=16000).to(device)
            input_values = ori_inputs.input_values.float() if device.type == "mps" else ori_inputs.input_values.half()
            with torch.no_grad():
                ori_outputs = model(input_values)
            S_ori = ori_outputs.last_hidden_state.float()
            return S_ori
    else:
...
def main(args):
    ...
    if f0_condition:
        ...
        F0_ori = torch.from_numpy(F0_ori).to(device=device, dtype=torch.float32)[None]
        F0_alt = torch.from_numpy(F0_alt).to(device=device, dtype=torch.float32)[None]

        voiced_F0_ori = F0_ori[F0_ori > 1]
        voiced_F0_alt = F0_alt[F0_alt > 1]

        log_f0_alt = torch.log(F0_alt + 1e-5).float()
        voiced_log_f0_ori = torch.log(voiced_F0_ori + 1e-5).float()
        voiced_log_f0_alt = torch.log(voiced_F0_alt + 1e-5).float()
        ...
        shifted_f0_alt = torch.exp(shifted_log_f0_alt).float()
